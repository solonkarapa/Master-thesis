...
/* The following is code to model the handgrip strength (univariatly)*/
...
proc means data = thesis.dataset2 n mean std;
	class timepoint;
	var hand_max;
	output out=sumstat_h mean=mean std=std stderr=stderr n=n;
run;
...
proc tabulate data=thesis.dataset2;
	...
	class id timepoint;
	var hand_max;
	table hand_max*n*f=5., timepoint/rts=15;
run;
...
/****************** Descriptive graphs for hand_max ******************/
/*** Individual profiles ***/
proc sgplot data=thesis.dataset2 /*(where=(id in (34, 4, 23)))*/;
series x=timepoint y=hand_max/group=id lineattrs=(color=black pattern=solid);
yaxis LABEL = "Max(Handgrip)(kg)";
run;

/*** Mean and SD ***/
proc sgplot data = thesis.dataset2;
title 'Empirical Summary Plot';	
vline timepoint / response = hand_max stat = mean limitstat = stddev limits = both;
yaxis LABEL = "Max(Handgrip)(kg)";
run;

/*** Mean and SE ***/
proc sgplot data = thesis.dataset2;
title 'Max(Handgrip) (SE) vs Timepoint';	
vline timepoint / response = hand_max stat = mean limitstat = stderr limits = both;
yaxis LABEL = "Max(Handgrip)(kg)";
run;
...
where timepoint < 8;
plot hand_max*timepoint=id/ haxis=axis1 vaxis=axis2 nolegend;
plot2 hand_max*timepoint/ noaxis;
...
axis2 label=(angle=90 "Hand grip (kg)") value=(h=1) offset=(0.1)cm order = 0 to 30 by 5;
run;

/*** Mean and SE by Gender ***/
proc sgplot data = thesis.dataset2;
title 'Empirical Summary Plot, By Gender';	
vline timepoint / response =hand_max group = gender stat = mean limitstat = stderr limits = both;
yaxis LABEL = "Max(Handgrip)(kg)";
run;
...
vline timepoint / response =hand_max group = treatment stat = mean;
yaxis LABEL = "Max(Handgrip)(kg)";
run;

/* Empirical standard deviations over time*/
proc gplot data=sumstat_h; /*sumstat from proc means before*/
plot std*timepoint/vaxis=axis1;
symbol1 i=none v=circle color=black height=2 r=1 ;
title 'SD over time';
axis1 minor=none label=(angle=90 'SD of Max(Handgrip)(kg)');
run;
...
proc sort data=thesis.dataset2;
	by id;
run;
proc transpose data=thesis.dataset2 out=wide3 name=column_that_was_transposed prefix=time;
	by id;
	id timepoint;	
	var hand_max;
run;
...
ods graphics on; 
proc corr data=wide3 plots=matrix /*NOPROB*/ /*NOSIMPLE*/  /*fisher*/ outp=CorrOutp;
	var time00--time130;
	ods select pearsoncorr; 
	ods output pearsoncorr=pearsoncorr_h; 
run;
ods graphics off;
...
data tri (drop=i obsno variable);
	set pearsoncorr_h (drop=ptime00--ptime130);
...
proc mixed data=piecewise;
	...
	model hand_max = treatment*timepoint gender*timepoint weight*timepoint age*timepoint/noint solution OUTPM=out_h residual;
	where timepoint<8;
run;

/*OLS residual profiles - from the model for above*/
goptions reset=all;
proc gplot data=out_h;
...
if y1<y2; vario=(v1-v2)**2/2; run;
/* Calculation of the total variance (=4.8714510) */
...
proc gplot data=out;
plot vario*distance=1 p_vario*distance=2/ overlay haxis=axis1 vaxis=axis2 vref=4.8714510 lvref=3;
...
axis2 label=(h=2 A=90 "v(u)") value=(h=1.5) minor=none order=(0 to 12);
title h=3 "Empirical Semi-variogram (Y3)";
run;
...
/*Model A*/
proc mixed data = out_h method = ml;
